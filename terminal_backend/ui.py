    ...
    with tab1:
        ...
        for patient in patient_orders:
            ...
            if patient[-1] == 0:
                ...
                process_patient = st.checkbox("Process patient", key=str(patient[0]) + "_process")
                system = System(databse_service=os.getenv("DATABASE_SERVICE"))
                if process_patient:
                ...
                view_pdf = st.checkbox("View patient record", key=str(patient[0]) + "_view")
                if view_pdf:
                    ...
                    try:
                        ...
                        raise FileNotFoundError("Record not found") from None
                st.write("-----------------------------------")
        ...
        for patient in patient_orders:
            if patient[-1] == 1:
                ...
                process_patient = st.checkbox("Uncheck patient", key=str(patient[0]) + "_uncheck")
                system = System(databse_service=os.getenv("DATABASE_SERVICE"))
                if process_patient:
